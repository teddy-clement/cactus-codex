...
export default function AppsPage() {
  const { currentApp: ctxApp } = useApp()
  ...
  if (!app) return <div className="panel panel-pad">Aucune application trouvée.</div>

  return (
    <div className="apps-shell">
      <section className="apps-header panel glass-panel">
        <div>
          <div className="phg">// {appName.toLowerCase()} control center</div>
          <h2 className="apps-title">Des modules pilotés proprement.</h2>
          <p className="apps-copy">Pilotez {appName} globalement ou module par module depuis ce panneau de contrôle.</p>
        </div>
      ...
      <article className="app-card panel app-card-focus">
        ...
        <div className="app-quick-actions">
          <button className={`control-btn ${app.status === 'maintenance' ? 'control-btn-live' : 'control-btn-maintenance'}`} onClick={() => patchApp({ status: app.status === 'maintenance' ? 'online' : 'maintenance', maintenance_message: draft.maintenance_message }, app.status === 'maintenance' ? `${appName} remis en ligne.` : `${appName} passe en maintenance.`)} disabled={saving === app.id}>{saving === app.id ? 'Traitement…' : app.status === 'maintenance' ? `Remettre ${appName} en ligne` : `Passer ${appName} en maintenance`}</button>
          <button className="control-btn control-btn-secondary" onClick={() => patchApp({ last_restart_at: 'now' }, `Redémarrage ${appName} confirmé.`)} disabled={saving === app.id}>Confirmer redémarrage</button>
        </div>
        ...
        <div className="app-footer-row">
          ...
          <button className="control-btn control-btn-primary" onClick={() => patchApp(draft, `Paramètres ${appName} enregistrés.`)} disabled={saving === app.id}>Enregistrer {appName}</button>
        </div>
      </article>

      <section className="panel panel-pad">
        <div className="ph"><div className="pht">Modules {appName}</div><div className="phg">// maintenance ciblée</div></div>
      ...
      <section className="panel panel-pad">
        <div className="ph"><div className="pht">Signaux structurels</div><div className="phg">// monitoring</div></div>
